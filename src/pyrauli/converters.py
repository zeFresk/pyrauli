...
def from_qiskit_qc(qiskit_circuit: QuantumCircuit, noise_model: pyrauli.NoiseModel = None) -> pyrauli.Circuit:
    ...
    for instruction in qiskit_circuit.data:
        ...
        # --- Decompositions ---
            ...
            pyrauli_circuit.rz(theta, target)
            pyrauli_circuit.h(target)
        elif op_name == "ry":
            ...
            # 1. Rz(pi/2)
            pyrauli_circuit.rz(math.pi / 2, target)
            # 2. H
            pyrauli_circuit.h(target)
            # 3. Rz(-theta)
            pyrauli_circuit.rz(-theta, target)
            ...
            pyrauli_circuit.rz(math.pi / 2, q0)
            pyrauli_circuit.rz(math.pi / 2, q1)
        elif op_name == "rzz":
